	...
	<dd>
		<table>
			...
			<tr>
				...
				<?php } ?>
				<h2 class="search__title">あなたに合ったサーバーを探す</h2>
				<td>
			...
			<tr>
				...
				<td>
					<ul class="list_widesp">
						...
						<?php }else{ ?>
						<li>
							<select name="bottle" id="">
								<option value="">ボトルの種類</option>
								<option value="disposable">ワンウェイ（使い捨て）方式</option>
								<option value="returnable">リターナブル（返却）方式</option>
								<option value="connection">コネクト（水道直結）方式</option>
							</select>
						</li>
						<li>
		...
		<dl id="search_more">
			<dt>
				...
				<?php }else{ ?>
					<div id="morebutton">
						<ion-icon name="add-circle"></ion-icon>さらにこだわって選びたい方はこちら
					</div>
				<?php } ?>
			</dt>
			<dd>
				<ul class="list_wide">
					...
					<li>
						<input type="checkbox" name="coffee" value="1" id="filtering06_12" <?php if (isset($_POST['coffee'])) { echo 'checked="checked"'; } ?>/>
...
-->
		...
		<ion-icon name="search"></ion-icon>ウォーターサーバー<span>を</span>検索
		</a>
		<?php } ?>
	</dd>
</dl>
