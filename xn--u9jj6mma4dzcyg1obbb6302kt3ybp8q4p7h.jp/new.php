<?php
  $redirect[1] = "https://h.accesstrade.net/sp/cc?rk=0100l9vx00bizl&add="; //shinano
  $redirect[2] = "https://h.accesstrade.net/sp/cc?rk=0100nbjp00bizl"; //kirala
  $redirect[3] = "https://track.affiliate-b.com/visit.php?guid=ON&a=t8195i-v2755388&p=0805307"; //aqua
  $redirect[4] = "https://track.affiliate-b.com/visit.php?guid=ON&a=q1398F-630765j&p=1506217y"; //crystal
  $redirect[5] = "https://track.affiliate-b.com/visit.php?guid=ON&a=M6645y-P223157X&p=15062"; //5ban

  $redirect[6] = "https://track.affiliate-b.com/visit.php?guid=ON&a=d2423k-l195973q&p=1506217y"; //energy
  $redirect[7] = "https://h.accesstrade.net/sp/cc?rk=0100o8bv00bizl"; //premium
  $redirect[8] = "http://h.accesstrade.net/sp/cc?rk=0100hnex006pd3"; //suntryサントリー天然水サーバー
  $redirect[9] = "http://h.accesstrade.net/sp/cc?rk=0100hnew006pd3"; //suntry-1サントリー天然水サーバー
  $redirect[10] = "http://h.accesstrade.net/sp/cc?rk=0100mpta00bizl"; //creclamio

  $redirect[11] = "https://h.accesstrade.net/sp/cc?rk=0100nxsv00bizl"; //fujinoyusui
  $redirect[12] = "http://h.accesstrade.net/sp/cc?rk=0100h3ml006pd3"; //ideal
  $redirect[13] = "http://ad.scadnet.com/8Ef5IF/i4/"; //kirin
  $redirect[14] = "http://px.a8.net/svt/ejp?a8mat=1ZIMUV+B0KSII+1Z2W+6FWRL"; //megumiwater
  $redirect[15] = "http://px.a8.net/svt/ejp?a8mat=2BF8BQ+7OB6RU+2M2E+5YRHD"; //sakura

  $redirect[16] = "http://h.accesstrade.net/sp/cc?rk=01005i4800bizl"; //elfile
  $redirect[17] = "http://h.accesstrade.net/sp/cc?rk=01003l4z00bizl"; //fujiseiryu
  $redirect[18] = "http://h.accesstrade.net/sp/cc?rk=0100cnwv00bizl"; //furele
  $redirect[19] = "https://h.accesstrade.net/sp/cc?rk=0100msf900bizl"; //clicla
  $redirect[20] = "http://www.rentracks.jp/adx/r.html?idx=0.1077.64977.934.1555&dna=24616"; //merqulop

  $redirect[21] = "http://h.accesstrade.net/sp/cc?rk=0100mxgv00bizl"; //suntry-2
  $redirect[22] = "https://h.accesstrade.net/sp/cc?rk=0100cp0500bizl"; //hawaiiwater
  $redirect[23] = "https://h.accesstrade.net/sp/cc?rk=0100fk1z006pd3&add="; //onewaywater
  $redirect[24] = "https://www.hikakude.jp/waterserver/lp/lp0015.html"; //mcmmegumi
  $redirect[25] = "https://track.affiliate-b.com/visit.php?guid=ON&a=y4590v-i136246H&p=0805307"; //urunon

  $redirect[26] = "http://h.accesstrade.net/sp/cc?rk=0100f44800bizl"; //mituuroko
  $redirect[27] = "http://h.accesstrade.net/sp/cc?rk=0100ezmr006pd3"; //aqua-magic
  $redirect[28] = "https://h.accesstrade.net/sp/cc?rk=0100erb300bizl"; //alpina-water
  $redirect[29] = "http://h.accesstrade.net/sp/cc?rk=0100ax0m006pd3"; //b-water
  $redirect[30] = "https://h.accesstrade.net/sp/cc?rk=0100im1l00bp82"; //aquaclaraアクアクララ

  $redirect[31] = "https://h.accesstrade.net/sp/cc?rk=0100nfqg00bp82"; //ベビアクア
  $redirect[32] = "http://h.accesstrade.net/sp/cc?rk=0100iudf00bp82"; //aquaclara-2アクアクララ
  $redirect[33] = "https://h.accesstrade.net/sp/cc?rk=0100ksxs006pd3"; //freciousフレシャス
  $redirect[34] = "https://h.accesstrade.net/sp/cc?rk=0100mvo8006pd3"; //frecious-1フレシャス
  $redirect[35] = "https://h.accesstrade.net/sp/cc?rk=0100jdv400bizl"; //clytia

  $redirect[36] = "https://h.accesstrade.net/sp/cc?rk=0100hg2v006pd3"; //cosmowater

  $redirect[37] = "http://ad.isaf.jp/bin/w.x?p=1794&b=544&o=530&s=535"; //aquaclara-akb
  $redirect[39] = "https://h.accesstrade.net/sp/cc?rk=0100nq54006pd3"; //freciousカフェ
  $redirect[40] = "https://h.accesstrade.net/sp/cc?rk=0100nt7o006pd3"; //freciousXUCC

  $redirect[41] = "https://h.accesstrade.net/sp/cc?rk=0100nm7n006pd3"; //frecious-norikae

  $redirect[42] = "https://h.accesstrade.net/sp/cc?rk=0100j1yo00bizl"; //premium MAM club
  
  $redirect[43] = "https://h.accesstrade.net/sp/cc?rk=0100ia8b006pd3"; //frecious dewo


  if( isset($_GET['id']) ){
    $id = intval($_GET['id']);
    header("Location:$redirect[$id]");
  }
?>
